...
<div class="form-group">
 <label class="font-italic text-center font-weight-bold"  for="manufactutedate">product ManufacturedDate:</label>
 <input type="date"  class="form-control-sm" id="manufactureddate" name="manufactutedate"  value="${existingobject.promnfdate}" required/>
</div>
<div class="form-group">-
 <label class="font-italic text-center font-weight-bold"  for="expirydate">product ExpiryDate:</label>
 <input type="date"  class="form-control-sm" id="expirydate" name="expirydate"  value="${existingobject.proexpdate}"  required/>
</div>
<div class="form-group">
 <label class="font-italic text-center font-weight-bold"  for="currentimage">current Image:</label>
 <img id="currentimage" src="data:image/jpeg;base64,${Base64.getEncoder().encodeToString(existingobject.proImage)}" alt="current  product  image" style="max-width:100px; max-height:100px;">  
 <input type="hidden" id="existingimage"  name="existingimage"  value="${Base64.getEncoder().encodeToString(existingobject.proImage)}"/>
</div>
<div class="form-group">
 <label class="font-italic text-center font-weight-bold"  for="current image">New Image:</label> 
...
<audio controls style="width: 200px;"><source src="data:audio/mpeg;base64,${Base64.getEncoder().encodeToString(existingobject.proaudio)}"></audio>
...
<video width="160" height="120" controls><source src="data:video/mp4;base64,${Base64.getEncoder().encodeToString(existingobject.provideo)}"></video>
